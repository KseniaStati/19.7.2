...
from settings import valid_email, valid_password, empty_password, empty_email, empty_auth_key, incorrect_auth_key
import os
...
def test_set_photo_pet(pet_photo="../image/453.jpg"):
...
def test_get_api_key_for_email_user_empty(email=empty_email, password=valid_password):
    """ Проверяем что запрос api ключа c невалидным значением логина и  валидным значением пароля возвращает статус 403
    и в результате не содержится слово key """
    # Отправляем запрос и сохраняем полученный ответ с кодом статуса в status, а текст ответа в result
    status, result = pf.get_api_key(email, password)
    # Сверяем полученный ответ с ожидаемым результатом
...
def test_get_api_key_for_email_and_password_user_empty(email=empty_email, password=empty_password):
    """ Проверяем что запрос api ключа c невалидным значением логина и невалидным значением пароля возвращает статус 403
    и в результате не содержится слово key """
    # Отправляем запрос и сохраняем полученный ответ с кодом статуса в status, а текст ответа в result
    status, result = pf.get_api_key(email, password)
    # Сверяем полученный ответ с ожидаемым результатом
